...
PHP_METHOD(Ice_Auth_Social_Adapter, __construct) {
	...
	zval _16$$8;
	zend_bool _11, _13, _6$$3;
	int ZEPHIR_LAST_CALL_STATUS;
	zephir_fcall_cache_entry *_1 = NULL;
	zval *config = NULL, config_sub, clientId, clientSecret, redirectUri, tmp, _10, _12, _14, _0$$3, _2$$3, _3$$3, _4$$3, _5$$3, _7$$3, _8$$4, _9$$4, _15$$8, _17$$8;
		zval this_zv;
	zval *this_ptr = getThis();
	if (EXPECTED(this_ptr)) {
		ZVAL_OBJ(&this_zv, Z_OBJ_P(this_ptr));
	...
	if (!(zephir_fast_count_int(config TSRMLS_CC))) {
		...
		ZVAL_NULL(&_4$$3);
		ZVAL_BOOL(&_5$$3, 1);
		ZEPHIR_CALL_METHOD(&_2$$3, &_0$$3, "get", NULL, 0, &_3$$3, &_4$$3, &_5$$3);
		zephir_check_call_status();
		ZEPHIR_INIT_NVAR(&_3$$3);
		ZVAL_STRING(&_3$$3, "auth");
		ZEPHIR_CALL_METHOD(&tmp, &_2$$3, "get", NULL, 0, &_3$$3);
		zephir_check_call_status();
		_6$$3 = zephir_is_true(&tmp);
		if (_6$$3) {
			zephir_read_property(&_4$$3, this_ptr, SL("provider"), PH_NOISY_CC | PH_READONLY);
			ZEPHIR_CALL_METHOD(&_7$$3, &tmp, "has", NULL, 0, &_4$$3);
			zephir_check_call_status();
			_6$$3 = zephir_is_true(&_7$$3);
		}
		if (_6$$3) {
			zephir_read_property(&_9$$4, this_ptr, SL("provider"), PH_NOISY_CC | PH_READONLY);
			ZEPHIR_CALL_METHOD(&_8$$4, &tmp, "get", NULL, 0, &_9$$4);
			zephir_check_call_status();
			ZEPHIR_CALL_METHOD(config, &_8$$4, "toarray", NULL, 0);
			zephir_check_call_status();
		}
	}
	...
	zephir_read_property(&_10, this_ptr, SL("clientId"), PH_NOISY_CC | PH_READONLY);
	_11 = !zephir_is_true(&_10);
	if (!(_11)) {
		zephir_read_property(&_12, this_ptr, SL("clientSecret"), PH_NOISY_CC | PH_READONLY);
		_11 = !zephir_is_true(&_12);
	}
	_13 = _11;
	if (!(_13)) {
		zephir_read_property(&_14, this_ptr, SL("redirectUri"), PH_NOISY_CC | PH_READONLY);
		_13 = !zephir_is_true(&_14);
	}
	if (_13) {
		ZEPHIR_INIT_VAR(&_15$$8);
		object_init_ex(&_15$$8, ice_exception_ce);
		ZEPHIR_INIT_VAR(&_16$$8);
		zephir_create_array(&_16$$8, 4, 0 TSRMLS_CC);
		ZEPHIR_INIT_VAR(&_17$$8);
		ZVAL_STRING(&_17$$8, "Option `%s`, `%s`, `%s` are required");
		zephir_array_fast_append(&_16$$8, &_17$$8);
		ZEPHIR_INIT_NVAR(&_17$$8);
		ZVAL_STRING(&_17$$8, "client_id");
		zephir_array_fast_append(&_16$$8, &_17$$8);
		ZEPHIR_INIT_NVAR(&_17$$8);
		ZVAL_STRING(&_17$$8, "client_secret");
		zephir_array_fast_append(&_16$$8, &_17$$8);
		ZEPHIR_INIT_NVAR(&_17$$8);
		ZVAL_STRING(&_17$$8, "redirect_uri");
		zephir_array_fast_append(&_16$$8, &_17$$8);
		ZEPHIR_CALL_METHOD(NULL, &_15$$8, "__construct", NULL, 2, &_16$$8);
		zephir_check_call_status();
		zephir_throw_exception_debug(&_15$$8, "ice/auth/social/adapter.zep", 60 TSRMLS_CC);
		ZEPHIR_MM_RESTORE();
		return;
	}
	ZEPHIR_MM_RESTORE();
...
static zend_object *zephir_init_properties_Ice_Auth_Social_Adapter(zend_class_entry *class_type TSRMLS_DC) {
