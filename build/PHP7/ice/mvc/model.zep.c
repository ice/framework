...
PHP_METHOD(Ice_Mvc_Model, __construct) {
	...
	zval data;
	zval *filters = NULL, filters_sub, *data_param = NULL, __$null, di, _1, _2, _3, _4, _5, _9, _7$$3, _8$$3, _10$$4, _11$$4, _12$$6;
		zval this_zv;
	zval *this_ptr = getThis();
	if (EXPECTED(this_ptr)) {
		ZVAL_OBJ(&this_zv, Z_OBJ_P(this_ptr));
	...
	ZEPHIR_INIT_NVAR(&_2);
	zephir_read_property(&_3, this_ptr, SL("fields"), PH_NOISY_CC | PH_READONLY);
	ZEPHIR_CALL_FUNCTION(&_5, "array_fill_keys", NULL, 9, &_3, &__$null);
	zephir_check_call_status();
	zephir_fast_array_merge(&_2, &_5, &data TSRMLS_CC);
	ZEPHIR_CPY_WRT(&data, &_2);
	ZEPHIR_CALL_PARENT(NULL, ice_mvc_model_ce, this_ptr, "__construct", &_6, 6, &data);
	zephir_check_call_status();
	zephir_read_property(&_4, this_ptr, SL("from"), PH_NOISY_CC | PH_READONLY);
	...
	if (!(zephir_is_true(&_9))) {
		zephir_read_property(&_10$$4, this_ptr, SL("db"), PH_NOISY_CC | PH_READONLY);
		...
		for (;_3$$3->funcs->valid(_3$$3 TSRMLS_CC) == SUCCESS && !EG(exception); _3$$3->funcs->move_forward(_3$$3 TSRMLS_CC)) {
			{
				...
				ZEPHIR_TMP_ITERATOR_PTR = _3$$3->funcs->get_current_data(_3$$3 TSRMLS_CC);
...
PHP_METHOD(Ice_Mvc_Model, fields) {
	...
	if (!(zephir_fast_count_int(&_0 TSRMLS_CC))) {
		if (!(zephir_fast_count_int(fields TSRMLS_CC))) {
			...
			ZEPHIR_CALL_FUNCTION(&_3$$5, "array_filter", &_4, 10, &_1$$5, &_2$$5);
			zephir_check_call_status();
			if (zephir_fast_count_int(&_3$$5 TSRMLS_CC)) {
				...
				zephir_check_call_status();
				ZEPHIR_CALL_FUNCTION(&_8$$7, "array_flip", &_9, 11, fields);
				zephir_check_call_status();
				ZEPHIR_CALL_FUNCTION(fields, "array_intersect_key", &_10, 12, &_7$$7, &_8$$7);
				zephir_check_call_status();
			}
		}
	} else {
		if (!(zephir_fast_count_int(fields TSRMLS_CC))) {
			...
			ZEPHIR_CALL_FUNCTION(&_13$$9, "array_flip", &_9, 11, &_12$$9);
			zephir_check_call_status();
			ZEPHIR_CALL_FUNCTION(fields, "array_intersect_key", &_10, 12, &_11$$9, &_13$$9);
			zephir_check_call_status();
		} else {
			...
			ZEPHIR_CALL_FUNCTION(&_16$$10, "array_filter", &_4, 10, &_14$$10, &_15$$10);
			zephir_check_call_status();
			if (zephir_fast_count_int(&_16$$10 TSRMLS_CC)) {
				...
				ZEPHIR_CALL_FUNCTION(&_20$$11, "array_flip", &_9, 11, &_19$$11);
				zephir_check_call_status();
				ZEPHIR_CALL_FUNCTION(fields, "array_intersect_key", &_10, 12, &_17$$11, &_20$$11);
				zephir_check_call_status();
			} else {
				ZEPHIR_CALL_METHOD(&_21$$12, this_ptr, "all", NULL, 0);
				zephir_check_call_status();
				ZEPHIR_CALL_FUNCTION(&_22$$12, "array_flip", &_9, 11, fields);
				zephir_check_call_status();
				zephir_read_property(&_23$$12, this_ptr, SL("fields"), PH_NOISY_CC | PH_READONLY);
				ZEPHIR_CALL_FUNCTION(&_24$$12, "array_flip", &_9, 11, &_23$$12);
				zephir_check_call_status();
				ZEPHIR_CALL_FUNCTION(fields, "array_intersect_key", &_10, 12, &_21$$12, &_22$$12, &_24$$12);
				zephir_check_call_status();
			}
		}
	}
...
PHP_METHOD(Ice_Mvc_Model, create) {
	...
	if (zephir_fast_count_int(&_6 TSRMLS_CC)) {
		...
		if (!(_8$$4)) {
			...
			ZEPHIR_CALL_METHOD(NULL, &_10$$5, "__construct", NULL, 13);
			zephir_check_call_status();
...
PHP_METHOD(Ice_Mvc_Model, update) {
	...
	if (Z_TYPE_P(&_0) == IS_ARRAY) {
		...
		zephir_is_iterable(&_1$$3, 0, "ice/mvc/model.zep", 335);
...
PHP_METHOD(Ice_Mvc_Model, remove) {
	...
	if (!(zephir_is_true(filters))) {
		...
		if (Z_TYPE_P(&_0$$3) == IS_ARRAY) {
			...
			zephir_is_iterable(&_1$$4, 0, "ice/mvc/model.zep", 437);
...
PHP_METHOD(Ice_Mvc_Model, exists) {
	...
	if (!(zephir_is_true(filters))) {
		...
		if (Z_TYPE_P(&_0$$3) == IS_ARRAY) {
			zephir_read_property(&_1$$4, this_ptr, SL("primary"), PH_NOISY_CC | PH_READONLY);
			zephir_is_iterable(&_1$$4, 0, "ice/mvc/model.zep", 468);
			ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&_1$$4), _2$$4)
			{
				ZEPHIR_INIT_NVAR(&key);
				ZVAL_COPY(&key, _2$$4);
				ZEPHIR_CALL_METHOD(&_3$$5, this_ptr, "has", &_4, 0, &key);
				zephir_check_call_status();
				_5$$5 = zephir_is_true(&_3$$5);
				if (_5$$5) {
					ZEPHIR_CALL_METHOD(&_6$$5, this_ptr, "get", &_7, 0, &key);
					zephir_check_call_status();
					_5$$5 = zephir_is_true(&_6$$5);
				}
				if (_5$$5) {
					ZEPHIR_CALL_METHOD(&_8$$6, this_ptr, "get", &_7, 0, &key);
					zephir_check_call_status();
					zephir_array_update_zval(filters, &key, &_8$$6, PH_COPY | PH_SEPARATE);
				} else {
					RETURN_MM_BOOL(0);
				}
			} ZEND_HASH_FOREACH_END();
			...
			zephir_read_property(&_10$$8, this_ptr, SL("primary"), PH_NOISY_CC | PH_READONLY);
			ZEPHIR_CALL_METHOD(&_9$$8, this_ptr, "has", &_4, 0, &_10$$8);
			zephir_check_call_status();
			_11$$8 = zephir_is_true(&_9$$8);
			if (_11$$8) {
				zephir_read_property(&_13$$8, this_ptr, SL("primary"), PH_NOISY_CC | PH_READONLY);
				ZEPHIR_CALL_METHOD(&_12$$8, this_ptr, "get", &_7, 0, &_13$$8);
				zephir_check_call_status();
			...
			if (_11$$8) {
				...
				zephir_read_property(&_14$$9, this_ptr, SL("primary"), PH_NOISY_CC);
				zephir_read_property(&_16$$9, this_ptr, SL("primary"), PH_NOISY_CC | PH_READONLY);
				ZEPHIR_CALL_METHOD(&_15$$9, this_ptr, "get", &_7, 0, &_16$$9);
				zephir_check_call_status();
				zephir_array_update_zval(filters, &_14$$9, &_15$$9, PH_COPY);
			} else {
				RETURN_MM_BOOL(0);
			}
		}
...
PHP_METHOD(Ice_Mvc_Model, getRelated) {
	...
	if (!(zephir_array_isset_fetch(&relation, &_0, &alias, 0 TSRMLS_CC))) {
		...
		ZEPHIR_CALL_FUNCTION(&_3$$3, "sprintf", NULL, 1, &_2$$3, &alias);
		zephir_check_call_status();
...
PHP_METHOD(Ice_Mvc_Model, getRules) {
	...
	if (zephir_is_true(fields)) {
		...
		if (Z_TYPE_P(fields) == IS_ARRAY) {
			...
			array_init(&rules);
			zephir_is_iterable(fields, 0, "ice/mvc/model.zep", 679);
			ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(fields), _2$$4)
			{
				...
				if (zephir_array_isset(&_3$$5, &field)) {
					...
					zephir_array_fetch(&_5$$6, &_4$$6, &field, PH_NOISY | PH_READONLY, "ice/mvc/model.zep", 675 TSRMLS_CC);
			...
			zephir_array_fetch(&_7$$7, &_6$$7, fields, PH_NOISY | PH_READONLY, "ice/mvc/model.zep", 681 TSRMLS_CC);
			RETURN_CTOR(_7$$7);
		}
	}
