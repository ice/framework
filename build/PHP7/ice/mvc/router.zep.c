...
PHP_METHOD(Ice_Mvc_Router, fastRoute) {
	...
	if (!(zephir_is_true(&_2))) {
		...
		zephir_is_iterable(&_12$$5, 0, "ice/mvc/router.zep", 119);
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&_12$$5), _13$$5)
		{
			...
			zephir_array_fetch_long(&_15$$6, &route, 0, PH_NOISY | PH_READONLY, "ice/mvc/router.zep", 117 TSRMLS_CC);
			zephir_array_fetch_long(&_16$$6, &route, 1, PH_NOISY | PH_READONLY, "ice/mvc/router.zep", 117 TSRMLS_CC);
	...
	zephir_array_fetch_string(&_23, &options, SL("cache"), PH_NOISY | PH_READONLY, "ice/mvc/router.zep", 128 TSRMLS_CC);
	if (zephir_is_true(&_23)) {
		...
		zephir_array_fetch_string(&_24$$8, &options, SL("cacheFile"), PH_NOISY | PH_READONLY, "ice/mvc/router.zep", 133 TSRMLS_CC);
		if ((zephir_file_exists(&_24$$8 TSRMLS_CC) == SUCCESS)) {
			zephir_array_fetch_string(&_25$$10, &options, SL("cacheFile"), PH_NOISY | PH_READONLY, "ice/mvc/router.zep", 134 TSRMLS_CC);
			...
			zephir_check_call_status();
			zephir_array_fetch_string(&_28$$11, &options, SL("cacheFile"), PH_NOISY | PH_READONLY, "ice/mvc/router.zep", 138 TSRMLS_CC);
...
PHP_METHOD(Ice_Mvc_Router, handle) {
	...
	do {
		if (ZEPHIR_IS_LONG(&_2, 0)) {
			...
			zephir_check_call_status();
			zephir_throw_exception_debug(&_8$$4, "ice/mvc/router.zep", 180 TSRMLS_CC);
			ZEPHIR_MM_RESTORE();
			return;
		}
		if (ZEPHIR_IS_LONG(&_2, 2)) {
			zephir_read_property(&_12$$6, this_ptr, SL("silent"), PH_NOISY_CC | PH_READONLY);
			if (zephir_is_true(&_12$$6)) {
				zephir_read_property(&_13$$7, this_ptr, SL("di"), PH_NOISY_CC | PH_READONLY);
			...
			zephir_check_call_status();
			zephir_throw_exception_debug(&_17$$6, "ice/mvc/router.zep", 190 TSRMLS_CC);
			ZEPHIR_MM_RESTORE();
			return;
		}
		if (ZEPHIR_IS_LONG(&_2, 1)) {
			...
			if (!(zephir_array_isset_string_fetch(&module, &holders, SL("module"), 0))) {
				...
				if (zephir_array_isset_string_fetch(&module, &data, SL("module"), 0)) {
					...
					ZVAL_STRING(&_22$$10, "");
					zephir_fast_str_replace(&_20$$10, &_21$$10, &_22$$10, &module TSRMLS_CC);
			...
			if (!(zephir_array_isset_string_fetch(&handler, &holders, SL("controller"), 0))) {
				...
				if (zephir_array_isset_string_fetch(&handler, &data, SL("controller"), 0)) {
					...
					ZVAL_STRING(&_25$$13, "");
					zephir_fast_str_replace(&_23$$13, &_24$$13, &_25$$13, &handler TSRMLS_CC);
			...
			if (!(zephir_array_isset_string_fetch(&action, &holders, SL("action"), 0))) {
				...
				if (zephir_array_isset_string_fetch(&action, &data, SL("action"), 0)) {
					...
					ZVAL_STRING(&_28$$16, "");
					zephir_fast_str_replace(&_26$$16, &_27$$16, &_28$$16, &action TSRMLS_CC);
