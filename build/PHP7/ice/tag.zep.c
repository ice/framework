	...
	if (zephir_is_true(&local)) {
		...
		if (!ZEPHIR_IS_FALSE_IDENTICAL(&action)) {
			...
			if (zephir_is_true(&_2$$6)) {
				zephir_read_property(&_4$$7, this_ptr, SL("di"), PH_NOISY_CC | PH_READONLY);
...
PHP_METHOD(Ice_Tag, script) {
	...
	ZEPHIR_INIT_VAR(&_11);
	ZVAL_STRING(&_11, "local");
	zephir_array_fast_append(&_10, &_11);
	ZEPHIR_INIT_NVAR(&_11);
	ZVAL_STRING(&_11, "content");
	zephir_array_fast_append(&_10, &_11);
	ZEPHIR_INIT_NVAR(&_11);
	ZVAL_STRING(&_11, "script");
	ZEPHIR_INIT_VAR(&_12);
	ZVAL_STRING(&_12, "content");
	ZVAL_BOOL(&_13, 1);
...
PHP_METHOD(Ice_Tag, select) {
	...
	if (ZEPHIR_IS_EMPTY(&options)) {
		...
		zephir_is_iterable(&options, 1, "ice/tag.zep", 662);
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&options), _6$$10, _7$$10, _5$$10)
		{
			...
			if (Z_TYPE_P(&text) == IS_ARRAY) {
				...
				zephir_is_iterable(&text, 0, "ice/tag.zep", 641);
...
PHP_METHOD(Ice_Tag, tagHtml) {
	...
	zephir_is_iterable(&defaultParams, 0, "ice/tag.zep", 698);
	...
	ZEPHIR_INIT_NVAR(&key);
	ZEPHIR_INIT_NVAR(&param);
	zephir_is_iterable(&defaultParams, 0, "ice/tag.zep", 702);
	...
	if (!(Z_TYPE_P(&content) == IS_UNDEF) && Z_STRLEN_P(&content)) {
		...
		_8$$9 = _7$$9;
		if (_8$$9) {
			zephir_array_fetch_string(&_10$$9, &attributes, SL("name"), PH_NOISY | PH_READONLY, "ice/tag.zep", 711 TSRMLS_CC);
			ZEPHIR_CALL_METHOD(&_9$$9, this_ptr, "hasvalue", NULL, 0, &_10$$9);
			zephir_check_call_status();
			_8$$9 = zephir_is_true(&_9$$9);
		}
		if (_8$$9) {
			zephir_array_fetch_string(&_11$$10, &attributes, SL("name"), PH_NOISY | PH_READONLY, "ice/tag.zep", 712 TSRMLS_CC);
...
PHP_METHOD(Ice_Tag, prepareTag) {
	...
	zval *name_param = NULL, *attributes_param = NULL, *skip_param = NULL, *single_param = NULL, order, keys, attrs, code, tmp, value, key, _0, _1, *_9, _3$$3, _4$$4, _5$$5, _8$$6, _15$$8, _17$$8, _18$$8, _19$$8, _24$$8, _20$$9, _21$$9, _22$$9, _23$$9, _25$$10, _26$$10;
	zval name, _2;
		zval this_zv;
	zval *this_ptr = getThis();
	if (EXPECTED(this_ptr)) {
	...
	if (zephir_array_isset_string_fetch(&tmp, &attrs, SL("name"), 0)) {
		...
		if (zephir_is_true(&_3$$3)) {
			...
			ZEPHIR_OBS_NVAR(&tmp);
			if (zephir_array_isset_string_fetch(&tmp, &attrs, SL("type"), 0)) {
	...
	zephir_is_iterable(&attrs, 0, "ice/tag.zep", 784);
	ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&attrs), _10, _11, _9)
	{
		ZEPHIR_INIT_NVAR(&key);
		if (_11 != NULL) { 
		...
		if (_14$$7) {
			zephir_read_property(&_15$$8, this_ptr, SL("escape"), PH_NOISY_CC | PH_READONLY);
			_16$$8 = zephir_is_true(&_15$$8);
			if (_16$$8) {
				zephir_read_property(&_17$$8, this_ptr, SL("di"), PH_NOISY_CC | PH_READONLY);
				ZEPHIR_INIT_NVAR(&_19$$8);
				ZVAL_STRING(&_19$$8, "filter");
				ZEPHIR_CALL_METHOD(&_18$$8, &_17$$8, "has", NULL, 0, &_19$$8);
				zephir_check_call_status();
				_16$$8 = zephir_is_true(&_18$$8);
			}
			if (_16$$8) {
				zephir_read_property(&_20$$9, this_ptr, SL("di"), PH_NOISY_CC | PH_READONLY);
				ZEPHIR_INIT_NVAR(&_22$$9);
				ZVAL_STRING(&_22$$9, "filter");
				ZEPHIR_CALL_METHOD(&_21$$9, &_20$$9, "get", NULL, 0, &_22$$9);
				zephir_check_call_status();
				ZEPHIR_INIT_NVAR(&_22$$9);
				ZVAL_STRING(&_22$$9, "escape");
	...
	if (single) {
		...
		zephir_read_property(&_26$$10, this_ptr, SL("docType"), PH_NOISY_CC | PH_READONLY);
...
PHP_METHOD(Ice_Tag, friendlyTitle) {
	...
	if (zephir_is_true(replace)) {
		...
		if (Z_TYPE_P(replace) == IS_ARRAY) {
			zephir_is_iterable(replace, 0, "ice/tag.zep", 915);
