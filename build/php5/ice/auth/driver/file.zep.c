...
PHP_METHOD(Ice_Auth_Driver_File, getUser) {
	...
	if (!(zephir_is_true(_0))) {
		ZEPHIR_CALL_PARENT(&username, ice_auth_driver_file_ce, getThis(), "getuser", &_1, 63, defaultValue);
...
PHP_METHOD(Ice_Auth_Driver_File, login) {
	...
	zval *password = NULL;
	zval *username, *password_param = NULL, *remember_param = NULL, *force_param = NULL, *user = NULL, *_1, *_3$$4 = NULL, *_5$$4, *_7$$6;
	...
	if (_0) {
		...
		ZEPHIR_CALL_PARENT(&_3$$4, ice_auth_driver_file_ce, getThis(), "checkhash", &_4, 64, password, _5$$4);
		zephir_check_call_status();
