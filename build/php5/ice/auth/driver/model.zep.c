...
PHP_METHOD(Ice_Auth_Driver_Model, autoLogin) {
	...
	if (zephir_is_true(token)) {
		...
		ZEPHIR_CALL_CE_STATIC(&token, ice_auth_driver_model_users_tokens_ce, "findone", &_2, 40, _3$$3);
		zephir_check_call_status();
		if (zephir_is_true(token)) {
			...
			if (_4$$4) {
				...
				zephir_check_call_status();
				ZEPHIR_CALL_FUNCTION(&_8$$4, "sha1", NULL, 41, _7$$4);
				zephir_check_call_status();
...
PHP_METHOD(Ice_Auth_Driver_Model, completeLogin) {
	...
	if (zephir_instance_of_ev(user, ice_auth_driver_model_users_ce TSRMLS_CC)) {
		...
		zephir_check_call_status();
		ZEPHIR_CALL_PARENT(NULL, ice_auth_driver_model_ce, getThis(), "completelogin", &_0, 42, _1$$3, roles);
		zephir_check_call_status();
	}
	ZEPHIR_MM_RESTORE();
...
PHP_METHOD(Ice_Auth_Driver_Model, getUser) {
	...
	if (!(zephir_is_true(_0))) {
		ZEPHIR_CALL_PARENT(&data, ice_auth_driver_model_ce, getThis(), "getuser", &_1, 39, defaultValue);
		zephir_check_call_status();
		...
		if (_9$$7) {
			...
			if (zephir_fast_in_array(&_14$$9, roles TSRMLS_CC)) {
				if (remember) {
					...
					ZEPHIR_CALL_METHOD(NULL, token, "__construct", NULL, 43);
					zephir_check_call_status();
					...
					zephir_check_call_status();
					ZEPHIR_CALL_FUNCTION(&_19$$12, "sha1", NULL, 41, _18$$12);
					zephir_check_call_status();
					...
					ZEPHIR_CALL_METHOD(&_22$$12, token, "create", NULL, 44);
					zephir_check_call_status();
...
PHP_METHOD(Ice_Auth_Driver_Model, loginBy) {
	...
	ZEPHIR_CALL_CE_STATIC(&userSocial, ice_auth_driver_model_users_social_ce, "findone", &_0, 40, _1);
	zephir_check_call_status();
		...
		if (_6$$3) {
			...
			if (zephir_fast_in_array(&_11$$4, roles TSRMLS_CC)) {
				if (remember) {
					...
					zephir_check_call_status();
					ZEPHIR_CALL_FUNCTION(&_16$$7, "sha1", NULL, 41, _15$$7);
					zephir_check_call_status();
					...
					ZEPHIR_CALL_METHOD(&_19$$7, token, "create", NULL, 44);
					zephir_check_call_status();
...
PHP_METHOD(Ice_Auth_Driver_Model, logout) {
	...
	if (zephir_is_true(token)) {
		...
		ZEPHIR_CALL_CE_STATIC(&token, ice_auth_driver_model_users_tokens_ce, "findone", &_4, 40, _5$$3);
		zephir_check_call_status();
		if (zephir_is_true(token)) {
			ZEPHIR_CALL_METHOD(NULL, token, "remove", NULL, 0);
			zephir_check_call_status();
	...
	if (logoutAll) {
		...
		ZEPHIR_CALL_METHOD(NULL, tokens, "__construct", NULL, 43);
		zephir_check_call_status();
		...
		if (zephir_is_true(user)) {
			...
			ZEPHIR_CALL_METHOD(NULL, tokens, "remove", NULL, 45, _6$$6);
			zephir_check_call_status();
		}
	}
	ZEPHIR_INIT_NVAR(_1);
	...
	ZEPHIR_RETURN_CALL_PARENT(ice_auth_driver_model_ce, getThis(), "logout", &_8, 46, _1);
	zephir_check_call_status();
	RETURN_MM();
...
PHP_METHOD(Ice_Auth_Driver_Model, refreshUser) {
	...
	if (!(zephir_is_true(user))) {
		...
		if (_1$$4) {
			...
			zephir_check_call_status();
			ZEPHIR_CALL_PARENT(NULL, ice_auth_driver_model_ce, getThis(), "completelogin", &_10, 42, _11$$5, roles);
			zephir_check_call_status();
