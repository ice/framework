...
PHP_METHOD(Ice_Auth_Social_Twitter, __construct) {
	...
	ZEPHIR_CALL_PARENT(NULL, ice_auth_social_twitter_ce, getThis(), "__construct", &_1, 72, config);
...
PHP_METHOD(Ice_Auth_Social_Twitter, authenticate) {
	...
	if (_0) {
		...
		if (zephir_is_true(_12$$3)) {
			...
			ZEPHIR_CALL_METHOD(&_16$$6, this_ptr, "prepareurlparams", &_4, 74, getDataUrl, params, _18$$6);
			zephir_check_call_status();
...
PHP_METHOD(Ice_Auth_Social_Twitter, prepareUrlParams) {
	...
	zephir_check_call_status();
	ZEPHIR_CALL_FUNCTION(&_5, "uniqid", NULL, 76, _4, ZEPHIR_GLOBAL(global_true));
	zephir_check_call_status();
	zephir_md5(_3, _5);
	...
	ZEPHIR_CALL_FUNCTION(&_14, "array_map", NULL, 49, &_11, params);
	zephir_check_call_status();
