...
PHP_METHOD(Ice_Config_Ini, __construct) {
	...
	ZEPHIR_CALL_FUNCTION(&ini, "parse_ini_file", &_0, 92, data, ZEPHIR_GLOBAL(global_true));
	zephir_check_call_status();
	ZEPHIR_SINIT_VAR(_1);
	ZVAL_LONG(&_1, 1);
	ZEPHIR_CALL_FUNCTION(&raw, "parse_ini_file", &_0, 92, data, ZEPHIR_GLOBAL(global_true), &_1);
	zephir_check_call_status();
	ZEPHIR_CALL_METHOD(&data, this_ptr, "map", NULL, 93, ini, raw);
	zephir_check_call_status();
	ZEPHIR_CALL_PARENT(NULL, ice_config_ini_ce, getThis(), "__construct", &_2, 46, data);
	zephir_check_call_status();
	ZEPHIR_MM_RESTORE();
	...
	for (
	  ...
	  ; zend_hash_move_forward_ex(_1, &_0)
		...
		if (Z_TYPE_P(value) == IS_ARRAY) {
			...
			ZEPHIR_CALL_METHOD(&_3$$4, this_ptr, "map", &_5, 93, value, _4$$4);
			zephir_check_call_status();
			...
			ZEPHIR_CALL_METHOD(&_6$$5, this_ptr, "cast", &_8, 94, value, _7$$5);
			zephir_check_call_status();
