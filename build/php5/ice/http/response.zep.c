...
PHP_METHOD(Ice_Http_Response, sendFile) {
	...
	if (ZEPHIR_IS_EMPTY(_0)) {
		...
		ZEPHIR_CALL_FUNCTION(&filepath, "realpath", NULL, 139, _3$$4);
		zephir_check_call_status();
		ZEPHIR_CALL_FUNCTION(&size, "filesize", NULL, 140, filepath);
		zephir_check_call_status();
		ZEPHIR_SINIT_VAR(_4$$4);
		ZVAL_STRING(&_4$$4, "rb", 0);
		ZEPHIR_CALL_FUNCTION(&file, "fopen", NULL, 141, filepath, &_4$$4);
		zephir_check_call_status();
	...
	zephir_check_call_status();
	while (1) {
		ZEPHIR_CALL_FUNCTION(&_33, "ob_get_level", &_34, 129);
		zephir_check_call_status();
		if (!(zephir_is_true(_33))) {
			break;
		}
		ZEPHIR_CALL_FUNCTION(NULL, "ob_end_flush", &_35, 142);
		zephir_check_call_status();
	}
	ZEPHIR_CALL_FUNCTION(NULL, "ignore_user_abort", NULL, 143, ZEPHIR_GLOBAL(global_true));
	zephir_check_call_status();
	...
	if (ZEPHIR_IS_EMPTY(_36)) {
		...
		ZVAL_LONG(&_37$$13, 0);
		ZEPHIR_CALL_FUNCTION(NULL, "set_time_limit", &_38, 144, &_37$$13);
		zephir_check_call_status();
	} else {
		...
		ZEPHIR_CALL_FUNCTION(NULL, "set_time_limit", &_38, 144, &_40$$14);
		zephir_check_call_status();
	}
	block = 16384;
	...
	ZEPHIR_CALL_FUNCTION(NULL, "fseek", NULL, 145, file, start);
	zephir_check_call_status();
	while (1) {
		if (!(!(zephir_feof(file TSRMLS_CC)))) {
			break;
		}
		ZEPHIR_CALL_FUNCTION(&pos, "ftell", &_43, 146, file);
		zephir_check_call_status();
		_44$$17 = ZEPHIR_GT(pos, end);
		if (!(_44$$17)) {
			ZEPHIR_CALL_FUNCTION(&_45$$17, "connection_aborted", &_46, 147);
			zephir_check_call_status();
	...
	if (zephir_is_true(isDelete)) {
		...
		/* try_start_1: */
			...
			ZEPHIR_CALL_FUNCTION(NULL, "unlink", NULL, 149, filepath);
