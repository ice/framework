...
PHP_METHOD(Ice_Log_Driver_File, __construct) {
	...
	if (!((zephir_file_exists(file TSRMLS_CC) == SUCCESS))) {
		ZEPHIR_CALL_FUNCTION(&_0$$3, "touch", NULL, 159, file);
	...
	ZEPHIR_CALL_FUNCTION(&_4, "is_writable", NULL, 160, file);
	zephir_check_call_status();
...
PHP_METHOD(Ice_Log_Driver_File, log) {
	...
	ZVAL_LONG(&_0, 8);
	ZEPHIR_CALL_FUNCTION(NULL, "file_put_contents", NULL, 161, _5, line, &_0);
	zephir_check_call_status();
	ZEPHIR_MM_RESTORE();
