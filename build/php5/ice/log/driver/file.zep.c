...
PHP_METHOD(Ice_Log_Driver_File, __construct) {
	...
	if (!((zephir_file_exists(file TSRMLS_CC) == SUCCESS))) {
		ZEPHIR_CALL_FUNCTION(&_0$$3, "touch", NULL, 121, file);
	...
	ZEPHIR_CALL_FUNCTION(&_4, "is_writable", NULL, 122, file);
	zephir_check_call_status();
...
PHP_METHOD(Ice_Log_Driver_File, log) {
	...
	ZEPHIR_CALL_FUNCTION(&_1, "date", NULL, 52, &_0);
	zephir_check_call_status();
	ZEPHIR_INIT_VAR(_2);
	...
	ZVAL_LONG(&_0, 8);
	ZEPHIR_CALL_FUNCTION(NULL, "file_put_contents", NULL, 123, _5, line, &_0);
	zephir_check_call_status();
	ZEPHIR_MM_RESTORE();
...
PHP_METHOD(Ice_Log_Driver_File, interpolate) {
	...
	ZEPHIR_RETURN_CALL_FUNCTION("strtr", NULL, 87, message, replace);
	zephir_check_call_status();
	RETURN_MM();
