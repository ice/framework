...
PHP_METHOD(Ice_Log_Driver_File, __construct) {
	...
	if (!((zephir_file_exists(file TSRMLS_CC) == SUCCESS))) {
		ZEPHIR_CALL_FUNCTION(&_0$$3, "touch", NULL, 156, file);
		...
		if (!(zephir_is_true(_0$$3))) {
			...
			ZEPHIR_CALL_METHOD(NULL, _1$$4, "__construct", &_3, 14, _2$$4);
			zephir_check_call_status();
	...
	ZEPHIR_CALL_FUNCTION(&_4, "is_writable", NULL, 157, file);
	zephir_check_call_status();
	if (!(zephir_is_true(_4))) {
		...
		ZEPHIR_CALL_METHOD(NULL, _5$$5, "__construct", &_3, 14, _6$$5);
		zephir_check_call_status();
...
PHP_METHOD(Ice_Log_Driver_File, log) {
	...
	ZEPHIR_CALL_FUNCTION(&_1, "date", NULL, 40, &_0);
	zephir_check_call_status();
	ZEPHIR_INIT_VAR(_2);
	...
	ZVAL_LONG(&_0, 8);
	ZEPHIR_CALL_FUNCTION(NULL, "file_put_contents", NULL, 158, _5, line, &_0);
	zephir_check_call_status();
	ZEPHIR_MM_RESTORE();
...
PHP_METHOD(Ice_Log_Driver_File, interpolate) {
	...
	ZEPHIR_RETURN_CALL_FUNCTION("strtr", NULL, 114, message, replace);
	zephir_check_call_status();
	RETURN_MM();
