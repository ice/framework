...
PHP_METHOD(Ice_Mvc_View_Engine_Sleet, compile) {
	...
	ZEPHIR_CALL_FUNCTION(&_7, "realpath", NULL, 138, _6);
	zephir_check_call_status();
	ZEPHIR_SINIT_VAR(_8);
	ZVAL_STRING(&_8, "", 0);
	...
	if (zephir_is_true(compile)) {
		...
		ZEPHIR_CALL_METHOD(NULL, _10$$9, "__construct", NULL, 173, this_ptr);
		zephir_check_call_status();
		...
		if (ZEPHIR_IS_TRUE_IDENTICAL(compile)) {
			...
			if (!(zephir_is_true(_13$$12))) {
				...
				ZEPHIR_CALL_FUNCTION(NULL, "mkdir", NULL, 58, dir, &_14$$13, ZEPHIR_GLOBAL(global_true));
				zephir_check_call_status();
