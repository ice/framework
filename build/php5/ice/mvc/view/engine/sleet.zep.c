...
PHP_METHOD(Ice_Mvc_View_Engine_Sleet, compile) {
	...
	ZEPHIR_INIT_VAR(_5);
	ZEPHIR_CALL_FUNCTION(&_6, "dirname", NULL, 51, path);
	zephir_check_call_status();
	ZEPHIR_CALL_FUNCTION(&_7, "realpath", NULL, 136, _6);
	zephir_check_call_status();
	ZEPHIR_SINIT_VAR(_8);
	ZVAL_STRING(&_8, "", 0);
	...
	ZEPHIR_CALL_FUNCTION(&file, "basename", NULL, 52, path, &_9);
	zephir_check_call_status();
	...
	do {
		...
		if (ZEPHIR_IS_LONG(compile, 2)) {
			if (!((zephir_file_exists(compiledPath TSRMLS_CC) == SUCCESS))) {
				...
				ZEPHIR_CALL_FUNCTION(&compile, "md5_file", NULL, 53, compiledPath);
				zephir_check_call_status();
			}
			break;
		}
	...
	if (zephir_is_true(compile)) {
		...
		ZEPHIR_CALL_METHOD(NULL, _10$$9, "__construct", NULL, 169, this_ptr);
		zephir_check_call_status();
		...
		if (ZEPHIR_IS_TRUE_IDENTICAL(compile)) {
			ZEPHIR_CALL_FUNCTION(&_13$$12, "is_dir", NULL, 54, dir);
			zephir_check_call_status();
			if (!(zephir_is_true(_13$$12))) {
				ZEPHIR_SINIT_VAR(_14$$13);
				ZVAL_LONG(&_14$$13, 0);
				ZEPHIR_CALL_FUNCTION(&old, "umask", &_15, 55, &_14$$13);
				zephir_check_call_status();
				ZEPHIR_SINIT_NVAR(_14$$13);
				ZVAL_LONG(&_14$$13, 0777);
				ZEPHIR_CALL_FUNCTION(NULL, "mkdir", NULL, 56, dir, &_14$$13, ZEPHIR_GLOBAL(global_true));
				zephir_check_call_status();
				ZEPHIR_CALL_FUNCTION(NULL, "umask", &_15, 55, old);
				zephir_check_call_status();
			}
...
PHP_METHOD(Ice_Mvc_View_Engine_Sleet, render) {
	...
	ZEPHIR_CALL_FUNCTION(NULL, "ob_start", NULL, 167);
	zephir_check_call_status();
	if (zephir_require_zval(path TSRMLS_CC) == FAILURE) {
		RETURN_MM_NULL();
	}
	ZEPHIR_RETURN_CALL_FUNCTION("ob_get_clean", NULL, 168);
	zephir_check_call_status();
	RETURN_MM();
