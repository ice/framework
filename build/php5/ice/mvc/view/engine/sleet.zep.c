...
PHP_METHOD(Ice_Mvc_View_Engine_Sleet, compile) {
	...
	if (zephir_is_true(compile)) {
		...
		ZEPHIR_CALL_METHOD(NULL, _10$$9, "__construct", NULL, 168, this_ptr);
		zephir_check_call_status();
...
PHP_METHOD(Ice_Mvc_View_Engine_Sleet, render) {
	...
	ZEPHIR_CALL_FUNCTION(NULL, "ob_start", NULL, 166);
	zephir_check_call_status();
	if (zephir_require_zval(path TSRMLS_CC) == FAILURE) {
		RETURN_MM_NULL();
	}
	ZEPHIR_RETURN_CALL_FUNCTION("ob_get_clean", NULL, 167);
	zephir_check_call_status();
	RETURN_MM();
