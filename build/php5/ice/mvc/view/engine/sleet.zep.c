...
PHP_METHOD(Ice_Mvc_View_Engine_Sleet, compile) {
	...
	ZEPHIR_CALL_FUNCTION(&_7, "realpath", NULL, 130, _6);
	zephir_check_call_status();
	ZEPHIR_SINIT_VAR(_8);
	ZVAL_STRING(&_8, "", 0);
	...
	ZEPHIR_CALL_FUNCTION(&file, "basename", NULL, 35, path, &_9);
	zephir_check_call_status();
	...
	do {
		...
		if (ZEPHIR_IS_LONG(compile, 2)) {
			if (!((zephir_file_exists(compiledPath TSRMLS_CC) == SUCCESS))) {
				...
				ZEPHIR_CALL_FUNCTION(&compile, "md5_file", NULL, 36, compiledPath);
				zephir_check_call_status();
			}
			break;
		}
	...
	if (zephir_is_true(compile)) {
		...
		ZEPHIR_CALL_METHOD(NULL, _10$$9, "__construct", NULL, 131, this_ptr);
		zephir_check_call_status();
		...
		if (ZEPHIR_IS_TRUE_IDENTICAL(compile)) {
			ZEPHIR_CALL_FUNCTION(&_13$$12, "is_dir", NULL, 37, dir);
			zephir_check_call_status();
			if (!(zephir_is_true(_13$$12))) {
				ZEPHIR_SINIT_VAR(_14$$13);
				ZVAL_LONG(&_14$$13, 0);
				ZEPHIR_CALL_FUNCTION(&old, "umask", &_15, 38, &_14$$13);
				zephir_check_call_status();
				ZEPHIR_SINIT_NVAR(_14$$13);
				ZVAL_LONG(&_14$$13, 0777);
				ZEPHIR_CALL_FUNCTION(NULL, "mkdir", NULL, 39, dir, &_14$$13, ZEPHIR_GLOBAL(global_true));
				zephir_check_call_status();
				ZEPHIR_CALL_FUNCTION(NULL, "umask", &_15, 38, old);
				zephir_check_call_status();
			}
