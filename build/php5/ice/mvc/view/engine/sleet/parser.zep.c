...
PHP_METHOD(Ice_Mvc_View_Engine_Sleet_Parser, __construct) {
	...
	ZEPHIR_INIT_NVAR(_0);
	ZVAL_LONG(_0, 256);
	ZEPHIR_CALL_METHOD(&methods, tag, "getmethods", NULL, 147, _0);
	zephir_check_call_status();
		...
		do {
			if (ZEPHIR_IS_LONG(ch, '{')) {
				...
				if (ZEPHIR_IS_FALSE_IDENTICAL(end)) {
					...
					ZEPHIR_CALL_FUNCTION(&_11$$5, "substr_count", &_12, 148, _9$$5, _10$$5);
					zephir_check_call_status();
			...
			if (ZEPHIR_IS_LONG(ch, '%')) {
				...
				if (ZEPHIR_IS_FALSE_IDENTICAL(end)) {
					...
					ZEPHIR_CALL_FUNCTION(&_33$$7, "substr_count", &_12, 148, _31$$7, _32$$7);
					zephir_check_call_status();
			...
			if (ZEPHIR_IS_LONG(ch, '#')) {
				...
				if (ZEPHIR_IS_FALSE_IDENTICAL(end)) {
					...
					ZEPHIR_CALL_FUNCTION(&_51$$9, "substr_count", &_12, 148, _49$$9, _50$$9);
					zephir_check_call_status();
...
PHP_METHOD(Ice_Mvc_View_Engine_Sleet_Parser, parse) {
	...
	ZEPHIR_CALL_FUNCTION(&tokenized, "token_get_all", NULL, 149, php);
	zephir_check_call_status();
	...
	do {
		...
		if (ZEPHIR_IS_LONG(_12, 330) || ZEPHIR_IS_LONG(_12, 332) || ZEPHIR_IS_LONG(_12, 304) || ZEPHIR_IS_LONG(_12, 305) || ZEPHIR_IS_LONG(_12, 320) || ZEPHIR_IS_LONG(_12, 323) || ZEPHIR_IS_LONG(_12, 321) || ZEPHIR_IS_LONG(_12, 325)) {
			...
			ZEPHIR_RETURN_CALL_METHOD(this_ptr, "parsecontrol", NULL, 150, _16$$12, tokens);
			zephir_check_call_status();
			RETURN_MM();
		}
		if (ZEPHIR_IS_LONG(_12, 319)) {
			ZEPHIR_RETURN_CALL_METHOD(this_ptr, "parseecho", NULL, 151, tokens);
			zephir_check_call_status();
			RETURN_MM();
		}
		if (ZEPHIR_IS_LONG(_12, 353)) {
			ZEPHIR_RETURN_CALL_METHOD(this_ptr, "parseset", &_17, 152, tokens);
			zephir_check_call_status();
			RETURN_MM();
		}
		...
		if (ZEPHIR_IS_LONG(_12, 344)) {
			ZEPHIR_RETURN_CALL_METHOD(this_ptr, "parseuse", NULL, 153, tokens);
			zephir_check_call_status();
			RETURN_MM();
		}
	} while(0);
...
PHP_METHOD(Ice_Mvc_View_Engine_Sleet_Parser, parseControl) {
	...
	ZEPHIR_CALL_METHOD(&_0, this_ptr, "doparse", NULL, 154, expression);
	zephir_check_call_status();
...
PHP_METHOD(Ice_Mvc_View_Engine_Sleet_Parser, doParse) {
	...
	ZVAL_STRING(prev, "", 1);
	while (1) {
		ZEPHIR_CALL_METHOD(&_0, i, "valid", &_1, 155);
		zephir_check_call_status();
		if (!(zephir_is_true(_0))) {
			break;
		}
		ZEPHIR_CALL_METHOD(&token, i, "current", &_2, 156);
		zephir_check_call_status();
		ZEPHIR_CALL_METHOD(&_4$$3, i, "key", &_5, 157);
		zephir_check_call_status();
		ZEPHIR_INIT_NVAR(_6$$3);
		ZVAL_LONG(_6$$3, (zephir_get_numberval(_4$$3) + 1));
		ZEPHIR_CALL_METHOD(&_3$$3, i, "offsetexists", &_7, 158, _6$$3);
		zephir_check_call_status();
		if (zephir_is_true(_3$$3)) {
			ZEPHIR_CALL_METHOD(&_8$$3, i, "key", &_5, 157);
			zephir_check_call_status();
			ZEPHIR_INIT_NVAR(_6$$3);
			ZVAL_LONG(_6$$3, (zephir_get_numberval(_8$$3) + 1));
			ZEPHIR_CALL_METHOD(&next, i, "offsetget", &_9, 159, _6$$3);
			zephir_check_call_status();
		} else {
		...
		if (ZEPHIR_IS_STRING(next, "|")) {
			ZEPHIR_CALL_METHOD(&_10$$4, i, "key", &_5, 157);
			zephir_check_call_status();
			ZEPHIR_INIT_NVAR(seek);
			ZVAL_LONG(seek, (zephir_get_numberval(_10$$4) + 2));
			ZEPHIR_CALL_METHOD(&filter, i, "offsetget", &_9, 159, seek);
			zephir_check_call_status();
			...
			if (zephir_fast_in_array(filter, filters TSRMLS_CC)) {
				ZEPHIR_CALL_METHOD(&_17$$5, this_ptr, "token", &_18, 160, token, prev, next);
				zephir_check_call_status();
				...
				ZEPHIR_CALL_METHOD(&_20$$6, this_ptr, "token", &_18, 160, token, prev, next);
				zephir_check_call_status();
			...
			ZEPHIR_CALL_METHOD(&_22$$4, i, "offsetexists", &_7, 158, _16$$4);
			zephir_check_call_status();
			if (zephir_is_true(_22$$4)) {
				ZEPHIR_INIT_NVAR(_16$$4);
				ZVAL_LONG(_16$$4, (zephir_get_numberval(seek) + 1));
				ZEPHIR_CALL_METHOD(&next, i, "offsetget", &_9, 159, _16$$4);
				zephir_check_call_status();
			} else {
				ZEPHIR_INIT_NVAR(next);
				ZVAL_NULL(next);
			...
			ZEPHIR_CALL_METHOD(NULL, i, "seek", &_23, 161, seek);
			zephir_check_call_status();
			ZEPHIR_CALL_METHOD(NULL, i, "next", &_24, 162);
			zephir_check_call_status();
			continue;
		}
		ZEPHIR_CALL_METHOD(&_25$$3, this_ptr, "token", &_18, 160, token, prev, next);
		zephir_check_call_status();
	...
	if (Z_TYPE_P(token) == IS_ARRAY) {
			...
			if (ZEPHIR_IS_LONG(_0$$3, 310)) {
				...
				do {
					...
					if (!(_13$$15)) {
						...
						ZEPHIR_CALL_FUNCTION(&_16$$15, "ctype_upper", NULL, 163, &_15$$15);
						zephir_check_call_status();
		...
		do {
			...
			if (ZEPHIR_IS_STRING(token, ":")) {
				...
				ZEPHIR_CALL_FUNCTION(&_20$$23, "end", &_21, 164, _19$$23);
				ZEPHIR_UNREF(_19$$23);
				zephir_check_call_status();
				do {
					if (ZEPHIR_IS_LONG(_20$$23, 1)) {
						...
						ZEPHIR_CALL_FUNCTION(NULL, "array_pop", &_23, 165, _22$$24);
						ZEPHIR_UNREF(_22$$24);
						zephir_check_call_status();
						RETURN_MM_STRING(" : ", 1);
					}
