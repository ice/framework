...
PHP_METHOD(Ice_Validation_Validator_File, validate) {
	...
	if (!(_32)) {
		...
		ZEPHIR_CALL_FUNCTION(&_34, "is_uploaded_file", NULL, 201, _33);
		zephir_check_call_status();
	...
	if (_32) {
		...
		zephir_create_array(replace, 1, 0 TSRMLS_CC);
		zephir_array_update_string(&replace, SL(":field"), &label, PH_COPY | PH_SEPARATE);
		ZEPHIR_CALL_FUNCTION(&_46$$10, "strtr", &_23, 113, message, replace);
		zephir_check_call_status();
	...
	if (_48) {
		...
		zephir_create_array(replace, 1, 0 TSRMLS_CC);
		zephir_array_update_string(&replace, SL(":field"), &label, PH_COPY | PH_SEPARATE);
		ZEPHIR_CALL_FUNCTION(&_60$$14, "strtr", &_23, 113, message, replace);
		zephir_check_call_status();
	...
	if (zephir_is_true(_61)) {
		...
		ZEPHIR_CALL_FUNCTION(&_67$$18, "floatval", &_68, 202, _66$$18);
		zephir_check_call_status();
		...
		ZEPHIR_CALL_FUNCTION(&_67$$18, "floatval", &_68, 202, _72$$18);
		zephir_check_call_status();
		ZEPHIR_CALL_FUNCTION(&_73$$18, "floatval", &_68, 202, bytes);
		zephir_check_call_status();
		if (ZEPHIR_GT(_67$$18, _73$$18)) {
			...
			ZEPHIR_CALL_FUNCTION(&_85$$20, "strtr", &_23, 113, message, replace);
			zephir_check_call_status();
	...
	if (zephir_is_true(_86)) {
		...
		if ((zephir_function_exists_ex(SS("finfo_open") TSRMLS_CC) == SUCCESS)) {
			...
			ZVAL_LONG(&_88$$26, 16);
			ZEPHIR_CALL_FUNCTION(&tmp, "finfo_open", NULL, 203, &_88$$26);
			zephir_check_call_status();
			zephir_array_fetch_string(&_89$$26, value, SL("tmp_name"), PH_NOISY | PH_READONLY, "ice/validation/validator/file.zep", 171 TSRMLS_CC);
			ZEPHIR_CALL_FUNCTION(&mime, "finfo_file", NULL, 204, tmp, _89$$26);
			zephir_check_call_status();
			ZEPHIR_CALL_FUNCTION(NULL, "finfo_close", NULL, 205, tmp);
			zephir_check_call_status();
		} else {
		...
		if (!(zephir_fast_in_array(mime, types TSRMLS_CC))) {
			...
			ZEPHIR_CALL_FUNCTION(&_101$$28, "strtr", &_23, 113, message, replace);
			zephir_check_call_status();
	...
	if (_103) {
		...
		ZEPHIR_CALL_FUNCTION(&tmp, "getimagesize", NULL, 206, _105$$32);
		zephir_check_call_status();
		...
		if (_110$$32) {
			...
			ZEPHIR_CALL_FUNCTION(&_122$$35, "strtr", &_23, 113, message, replace);
			zephir_check_call_status();
		...
		if (zephir_is_true(_123$$32)) {
			...
			if (_126$$39) {
				...
				ZEPHIR_CALL_FUNCTION(&_138$$40, "strtr", &_23, 113, message, replace);
				zephir_check_call_status();
