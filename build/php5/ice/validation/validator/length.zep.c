...
PHP_METHOD(Ice_Validation_Validator_Length, validate) {
	...
	if ((zephir_function_exists_ex(SS("mb_strlen") TSRMLS_CC) == SUCCESS)) {
		ZEPHIR_CALL_FUNCTION(&length, "mb_strlen", NULL, 184, value);
		zephir_check_call_status();
	} else {
	...
	if (_10) {
		...
		ZEPHIR_CALL_FUNCTION(&_22$$12, "strtr", &_23, 85, message, replace);
		zephir_check_call_status();
	...
	if (_24) {
		...
		ZEPHIR_CALL_FUNCTION(&_36$$16, "strtr", &_23, 85, message, replace);
		zephir_check_call_status();
