...
PHP_METHOD(Ice_Validation_Validator_Url, validate) {
	...
	ZEPHIR_SINIT_VAR(_1);
	ZVAL_LONG(&_1, 273);
	ZEPHIR_CALL_FUNCTION(&_2, "filter_var", NULL, 129, value, &_1);
	zephir_check_call_status();
	if (!(zephir_is_true(_2))) {
		...
		if (zephir_is_true(_6$$4)) {
			...
			ZVAL_STRING(_8$$8, "url", ZEPHIR_TEMP_PARAM_COPY);
		...
		ZEPHIR_CALL_FUNCTION(&_16$$4, "strtr", NULL, 113, message, replace);
		zephir_check_call_status();
