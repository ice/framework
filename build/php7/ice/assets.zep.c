...
PHP_METHOD(Ice_Assets, prepare) {
	...
	zephir_check_call_status();
	ZEPHIR_CALL_FUNCTION(&_2, "dirname", &_3, 48, &uri);
	zephir_check_call_status();
	ZEPHIR_INIT_VAR(&dir);
	ZEPHIR_CONCAT_VS(&dir, &_2, "/");
	ZEPHIR_INIT_VAR(&_4);
	ZEPHIR_CONCAT_SV(&_4, ".", &type);
	ZEPHIR_CALL_FUNCTION(&file, "basename", NULL, 49, &uri, &_4);
	zephir_check_call_status();
	...
	do {
		...
		if (ZEPHIR_IS_LONG(minify, 2)) {
			if (!((zephir_file_exists(&destination TSRMLS_CC) == SUCCESS))) {
				...
				ZEPHIR_CALL_FUNCTION(minify, "md5_file", NULL, 50, &destination);
				zephir_check_call_status();
			}
			break;
		}
	...
	if (!(zephir_is_true(minify))) {
		...
		if (ZEPHIR_IS_TRUE_IDENTICAL(minify)) {
			ZEPHIR_CALL_FUNCTION(&_11$$14, "dirname", &_3, 48, &destination);
			zephir_check_call_status();
			ZEPHIR_CALL_FUNCTION(&_12$$14, "is_dir", NULL, 51, &_11$$14);
			zephir_check_call_status();
			if (!(zephir_is_true(&_12$$14))) {
				ZVAL_LONG(&_13$$15, 0);
				ZEPHIR_CALL_FUNCTION(&old, "umask", &_14, 52, &_13$$15);
				zephir_check_call_status();
				ZEPHIR_CALL_FUNCTION(&_15$$15, "dirname", &_3, 48, &destination);
				zephir_check_call_status();
				ZVAL_LONG(&_13$$15, 0777);
				ZEPHIR_CALL_FUNCTION(NULL, "mkdir", NULL, 53, &_15$$15, &_13$$15, &__$true);
				zephir_check_call_status();
				ZEPHIR_CALL_FUNCTION(NULL, "umask", &_14, 52, &old);
				zephir_check_call_status();
			}
