	...
	if (zephir_is_true(&token)) {
		...
		if (zephir_is_true(&token)) {
			...
			if (_4$$4) {
				...
				zephir_check_call_status();
				ZEPHIR_CALL_FUNCTION(&_8$$4, "sha1", NULL, 39, &_7$$4);
				zephir_check_call_status();
...
PHP_METHOD(Ice_Auth_Driver_Model, completeLogin) {
	...
	if (zephir_instance_of_ev(user, ice_auth_driver_model_users_ce TSRMLS_CC)) {
		...
		zephir_check_call_status();
		ZEPHIR_CALL_PARENT(NULL, ice_auth_driver_model_ce, this_ptr, "completelogin", &_0, 0, &_1$$3, &roles);
		zephir_check_call_status();
	}
	ZEPHIR_MM_RESTORE();
...
PHP_METHOD(Ice_Auth_Driver_Model, getUser) {
	...
	if (!(zephir_is_true(&_0))) {
		ZEPHIR_CALL_PARENT(&data, ice_auth_driver_model_ce, this_ptr, "getuser", &_1, 0, defaultValue);
		zephir_check_call_status();
		...
		if (_9$$7) {
			...
			if (zephir_fast_in_array(&_14$$9, &roles TSRMLS_CC)) {
				if (remember) {
					...
					ZEPHIR_CALL_METHOD(NULL, &token, "__construct", NULL, 40);
					zephir_check_call_status();
					ZEPHIR_CALL_METHOD(&_16$$12, &user, "getid", NULL, 0);
					zephir_check_call_status();
					zephir_update_property_zval(&token, SL("user_id"), &_16$$12);
					zephir_read_property(&_17$$12, this_ptr, SL("request"), PH_NOISY_CC | PH_READONLY);
					ZEPHIR_CALL_METHOD(&_18$$12, &_17$$12, "getuseragent", NULL, 0);
					zephir_check_call_status();
					ZEPHIR_CALL_FUNCTION(&_19$$12, "sha1", NULL, 39, &_18$$12);
					zephir_check_call_status();
					zephir_update_property_zval(&token, SL("useragent"), &_19$$12);
					ZEPHIR_INIT_NVAR(&_15$$12);
					zephir_time(&_15$$12);
					zephir_update_property_zval(&token, SL("created"), &_15$$12);
					ZEPHIR_INIT_VAR(&_20$$12);
					zephir_time(&_20$$12);
					ZEPHIR_INIT_VAR(&_21$$12);
					zephir_add_function(&_21$$12, &_20$$12, &lifetime);
					zephir_update_property_zval(&token, SL("expires"), &_21$$12);
					ZEPHIR_CALL_METHOD(&_22$$12, &token, "create", NULL, 41);
					zephir_check_call_status();
					if (ZEPHIR_IS_TRUE_IDENTICAL(&_22$$12)) {
						zephir_read_property(&_23$$13, this_ptr, SL("cookies"), PH_NOISY_CC | PH_READONLY);
		...
		if (_6$$3) {
			...
			if (zephir_fast_in_array(&_11$$4, &roles TSRMLS_CC)) {
				if (remember) {
					...
					zephir_check_call_status();
					ZEPHIR_CALL_FUNCTION(&_16$$7, "sha1", NULL, 39, &_15$$7);
					zephir_check_call_status();
					...
					ZEPHIR_CALL_METHOD(&_19$$7, &token, "create", NULL, 41);
					zephir_check_call_status();
...
PHP_METHOD(Ice_Auth_Driver_Model, logout) {
	...
	if (logoutAll) {
		...
		ZEPHIR_CALL_METHOD(NULL, &tokens, "__construct", NULL, 40);
		zephir_check_call_status();
		...
		if (zephir_is_true(&user)) {
			...
			ZEPHIR_CALL_METHOD(NULL, &tokens, "remove", NULL, 42, &_6$$6);
			zephir_check_call_status();
		}
	}
	if (destroy) {
	...
	ZEPHIR_RETURN_CALL_PARENT(ice_auth_driver_model_ce, this_ptr, "logout", &_8, 0, &_9);
	zephir_check_call_status();
	RETURN_MM();
...
PHP_METHOD(Ice_Auth_Driver_Model, refreshUser) {
	...
	if (!(zephir_is_true(&user))) {
		...
		if (_1$$4) {
			...
			zephir_check_call_status();
			ZEPHIR_CALL_PARENT(NULL, ice_auth_driver_model_ce, this_ptr, "completelogin", &_10, 0, &_11$$5, &roles);
			zephir_check_call_status();
