	...
	if (zephir_is_true(&token)) {
		...
		if (zephir_is_true(&token)) {
			...
			if (_4$$4) {
				...
				zephir_check_call_status();
				ZEPHIR_CALL_FUNCTION(&_8$$4, "sha1", NULL, 55, &_7$$4);
				zephir_check_call_status();
...
PHP_METHOD(Ice_Auth_Driver_Model, completeLogin) {
	...
	if (zephir_instance_of_ev(user, ice_auth_driver_model_users_ce TSRMLS_CC)) {
		...
		zephir_check_call_status();
		ZEPHIR_CALL_PARENT(NULL, ice_auth_driver_model_ce, getThis(), "completelogin", &_0, 0, &_1$$3, &roles);
		zephir_check_call_status();
	}
	ZEPHIR_MM_RESTORE();
...
PHP_METHOD(Ice_Auth_Driver_Model, getUser) {
	...
	if (!(zephir_is_true(&_0))) {
		ZEPHIR_CALL_PARENT(&data, ice_auth_driver_model_ce, getThis(), "getuser", &_1, 0, defaultValue);
		zephir_check_call_status();
		...
		if (_9$$7) {
			...
			if (zephir_fast_in_array(&_14$$9, &roles TSRMLS_CC)) {
				if (remember) {
					...
					ZEPHIR_CALL_METHOD(NULL, &token, "__construct", NULL, 56);
					zephir_check_call_status();
					...
					zephir_check_call_status();
					ZEPHIR_CALL_FUNCTION(&_19$$12, "sha1", NULL, 55, &_18$$12);
					zephir_check_call_status();
					...
					ZEPHIR_CALL_METHOD(&_22$$12, &token, "create", NULL, 57);
					zephir_check_call_status();
		...
		if (_6$$3) {
			...
			if (zephir_fast_in_array(&_11$$4, &roles TSRMLS_CC)) {
				if (remember) {
					...
					zephir_check_call_status();
					ZEPHIR_CALL_FUNCTION(&_16$$7, "sha1", NULL, 55, &_15$$7);
					zephir_check_call_status();
					...
					ZEPHIR_CALL_METHOD(&_19$$7, &token, "create", NULL, 57);
					zephir_check_call_status();
...
PHP_METHOD(Ice_Auth_Driver_Model, logout) {
	...
	if (logoutAll) {
		...
		ZEPHIR_CALL_METHOD(NULL, &tokens, "__construct", NULL, 56);
		zephir_check_call_status();
		...
		if (zephir_is_true(&user)) {
			...
			ZEPHIR_CALL_METHOD(NULL, &tokens, "remove", NULL, 58, &_6$$6);
			zephir_check_call_status();
		}
	}
	if (destroy) {
	...
	ZEPHIR_RETURN_CALL_PARENT(ice_auth_driver_model_ce, getThis(), "logout", &_8, 0, &_9);
	zephir_check_call_status();
	RETURN_MM();
...
PHP_METHOD(Ice_Auth_Driver_Model, refreshUser) {
	...
	if (!(zephir_is_true(&user))) {
		...
		if (_1$$4) {
			...
			zephir_check_call_status();
			ZEPHIR_CALL_PARENT(NULL, ice_auth_driver_model_ce, getThis(), "completelogin", &_10, 0, &_11$$5, &roles);
			zephir_check_call_status();
