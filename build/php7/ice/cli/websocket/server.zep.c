...
PHP_METHOD(Ice_Cli_Websocket_Server, __construct) {
	...
	ZEPHIR_CALL_FUNCTION(&context, "stream_context_create", NULL, 77);
	zephir_check_call_status();
	if (zephir_fast_count_int(&options TSRMLS_CC)) {
		...
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&options), _4$$4, _5$$4, _3$$4)
		{
			...
			ZEPHIR_CALL_FUNCTION(NULL, "stream_context_set_option", &_7, 78, &context, &_6$$5, &key, &value);
			zephir_check_call_status();
	...
	ZEPHIR_MAKE_REF(&_15);
	ZEPHIR_MAKE_REF(&_16);
	ZEPHIR_CALL_FUNCTION(&_18, "stream_socket_server", NULL, 79, &_14, &_15, &_16, &_17, &context);
	ZEPHIR_UNREF(&_15);
	ZEPHIR_UNREF(&_16);
	zephir_check_call_status();
...
PHP_METHOD(Ice_Cli_Websocket_Server, run) {
	...
	if (zephir_array_isset_string(&_0, SL("boot"))) {
		...
		ZEPHIR_CALL_FUNCTION(NULL, "call_user_func", &_3, 75, &_2$$3, this_ptr);
		zephir_check_call_status();
	}
	while (1) {
		if (!(1)) {
		...
		if (zephir_array_isset_string(&_4$$4, SL("tick"))) {
			...
			ZEPHIR_CALL_FUNCTION(&_7$$5, "call_user_func", &_3, 75, &_6$$5, this_ptr);
			zephir_check_call_status();
		...
		ZEPHIR_CALL_FUNCTION(&stream, "stream_select", &_10, 76, &changed, &write, &except, &_8$$4);
		...
		if (ZEPHIR_GT_LONG(&stream, 0)) {
			...
			ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&changed), _11$$7)
			{
				...
				if (ZEPHIR_IS_IDENTICAL(&socket, &_12$$8)) {
					...
					ZEPHIR_CALL_FUNCTION(&tmp, "stream_socket_accept", &_14, 80, &_13$$9);
					zephir_check_call_status();
					if (!ZEPHIR_IS_FALSE_IDENTICAL(&tmp)) {
						...
						if (zephir_is_true(&_15$$10)) {
							...
							if (zephir_array_isset_string(&_17$$11, SL("connect"))) {
								...
								ZEPHIR_CALL_FUNCTION(NULL, "call_user_func", &_3, 75, &_19$$12, &_21$$12, this_ptr);
								zephir_check_call_status();
							}
						}
					}
					...
					if (_23$$13) {
						...
						if (zephir_array_isset_string(&_26$$14, SL("disconnect"))) {
							...
							ZEPHIR_CALL_FUNCTION(NULL, "call_user_func", &_3, 75, &_28$$15, &_30$$15, this_ptr);
							zephir_check_call_status();
						}
			...
			ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&messages), _35$$7)
			{
				...
				if (zephir_array_isset_string(&_36$$17, SL("message"))) {
					...
					ZEPHIR_CALL_FUNCTION(NULL, "call_user_func", &_3, 75, &_38$$18, &_39$$18, &_40$$18, this_ptr);
					zephir_check_call_status();
				}
			} ZEND_HASH_FOREACH_END();
			ZEPHIR_INIT_NVAR(&message);
...
PHP_METHOD(Ice_Cli_Websocket_Server, connect) {
	...
	ZEPHIR_INIT_VAR(&_4);
	ZVAL_STRING(&_4, " ");
	ZEPHIR_CALL_FUNCTION(&_5, "preg_replace", NULL, 81, &_2, &_4, &headers);
	zephir_check_call_status();
	...
	if (_36) {
		...
		ZEPHIR_CALL_FUNCTION(&_39, "call_user_func", NULL, 75, &_38, &client, this_ptr);
		zephir_check_call_status();
	...
	ZEPHIR_CALL_FUNCTION(&_46, "sha1", NULL, 57, &_43, &__$true);
	zephir_check_call_status();
