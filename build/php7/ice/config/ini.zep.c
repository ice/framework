...
PHP_METHOD(Ice_Config_Ini, __construct) {
	...
	ZEPHIR_CALL_FUNCTION(&ini, "parse_ini_file", &_0, 80, data, &__$true);
	zephir_check_call_status();
	ZVAL_LONG(&_1, 1);
	ZEPHIR_CALL_FUNCTION(&raw, "parse_ini_file", &_0, 80, data, &__$true, &_1);
	zephir_check_call_status();
	ZEPHIR_CALL_METHOD(data, this_ptr, "map", NULL, 81, &ini, &raw);
	zephir_check_call_status();
...
PHP_METHOD(Ice_Config_Ini, map) {
	...
	ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(ini), _1, _2, _0)
	{
		...
		if (Z_TYPE_P(&value) == IS_ARRAY) {
			...
			ZEPHIR_CALL_METHOD(&_3$$4, this_ptr, "map", &_5, 81, &value, &_4$$4);
			zephir_check_call_status();
			...
			ZEPHIR_CALL_METHOD(&_6$$5, this_ptr, "cast", &_8, 82, &value, &_7$$5);
			zephir_check_call_status();
