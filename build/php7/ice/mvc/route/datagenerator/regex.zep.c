...
PHP_METHOD(Ice_Mvc_Route_DataGenerator_Regex, generateVariableRouteData) {
	...
	ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&_0), _2, _3, _1)
	{
		...
		zephir_check_call_status();
		ZEPHIR_CALL_FUNCTION(&chunks, "array_chunk", &_6, 44, &regexToRoutesMap, &chunkSize, &__$true);
		zephir_check_call_status();
		...
		ZEPHIR_CALL_FUNCTION(&_9$$3, "array_map", &_10, 45, &_7$$3, &chunks);
		zephir_check_call_status();
...
PHP_METHOD(Ice_Mvc_Route_DataGenerator_Regex, computeChunkSize) {
	...
	ZVAL_LONG(&_4, 1);
	ZEPHIR_CALL_FUNCTION(&numParts, "max", NULL, 46, &_4, &_2);
	zephir_check_call_status();
	ZEPHIR_INIT_VAR(&_5);
...
PHP_METHOD(Ice_Mvc_Route_DataGenerator_Regex, addVariableRoute) {
	...
	ZEPHIR_CALL_METHOD(NULL, &_10, "__construct", NULL, 47, httpMethod, handler, &regex, &variables);
	zephir_check_call_status();
...
PHP_METHOD(Ice_Mvc_Route_DataGenerator_Regex, buildRegexForRoute) {
	...
	ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(routeData), _0)
	{
		...
		if (Z_TYPE_P(&part) == IS_STRING) {
			...
			ZVAL_STRING(&_1$$4, "~");
			ZEPHIR_CALL_FUNCTION(&_2$$4, "preg_quote", &_3, 48, &part, &_1$$4);
			zephir_check_call_status();
