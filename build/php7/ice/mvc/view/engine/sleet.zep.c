...
PHP_METHOD(Ice_Mvc_View_Engine_Sleet, compile) {
	...
	ZEPHIR_CALL_FUNCTION(&_7, "realpath", NULL, 126, &_6);
	zephir_check_call_status();
	ZEPHIR_INIT_VAR(&_8);
	ZVAL_STRING(&_8, "");
	...
	if (zephir_is_true(&compile)) {
		...
		ZEPHIR_CALL_METHOD(NULL, &_10$$9, "__construct", NULL, 158, this_ptr);
		zephir_check_call_status();
...
PHP_METHOD(Ice_Mvc_View_Engine_Sleet, render) {
	...
	ZEPHIR_INIT_NVAR(&value);
	ZEPHIR_INIT_NVAR(&key);
	ZEPHIR_CALL_FUNCTION(NULL, "ob_start", NULL, 156);
	zephir_check_call_status();
	if (zephir_require_zval(&path TSRMLS_CC) == FAILURE) {
		RETURN_MM_NULL();
	}
	ZEPHIR_RETURN_CALL_FUNCTION("ob_get_clean", NULL, 157);
	zephir_check_call_status();
	RETURN_MM();
