...
PHP_METHOD(Ice_Mvc_View_Engine_Sleet, compile) {
	...
	ZEPHIR_CALL_FUNCTION(&_7, "realpath", NULL, 118, &_6);
	zephir_check_call_status();
	ZEPHIR_INIT_VAR(&_8);
	ZVAL_STRING(&_8, "");
	...
	if (zephir_is_true(&compile)) {
		...
		ZEPHIR_CALL_METHOD(NULL, &_10$$9, "__construct", NULL, 119, this_ptr);
		zephir_check_call_status();
		...
		if (ZEPHIR_IS_TRUE_IDENTICAL(&compile)) {
			...
			if (!(zephir_is_true(&_13$$12))) {
				...
				ZEPHIR_CALL_FUNCTION(NULL, "mkdir", NULL, 38, &dir, &_14$$13, &__$true);
				zephir_check_call_status();
...
PHP_METHOD(Ice_Mvc_View_Engine_Sleet, render) {
	...
	ZEPHIR_RETURN_CALL_FUNCTION("ob_get_clean", NULL, 117);
	zephir_check_call_status();
	RETURN_MM();
