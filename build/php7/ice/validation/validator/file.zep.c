...
PHP_METHOD(Ice_Validation_Validator_File, validate) {
	...
	if (_8) {
		...
		ZEPHIR_CALL_FUNCTION(&_22$$5, "strtr", &_23, 76, &message, &replace);
		zephir_check_call_status();
	...
	if (!(_32)) {
		...
		ZEPHIR_CALL_FUNCTION(&_34, "is_uploaded_file", NULL, 152, &_33);
		zephir_check_call_status();
	...
	if (_32) {
		...
		zephir_create_array(&replace, 1, 0 TSRMLS_CC);
		zephir_array_update_string(&replace, SL(":field"), &label, PH_COPY | PH_SEPARATE);
		ZEPHIR_CALL_FUNCTION(&_46$$10, "strtr", &_23, 76, &message, &replace);
		zephir_check_call_status();
	...
	if (_48) {
		...
		zephir_create_array(&replace, 1, 0 TSRMLS_CC);
		zephir_array_update_string(&replace, SL(":field"), &label, PH_COPY | PH_SEPARATE);
		ZEPHIR_CALL_FUNCTION(&_60$$14, "strtr", &_23, 76, &message, &replace);
		zephir_check_call_status();
	...
	if (zephir_is_true(&_61)) {
		...
		ZEPHIR_CALL_FUNCTION(&_67$$18, "floatval", &_68, 153, &_66$$18);
		zephir_check_call_status();
		...
		ZEPHIR_CALL_FUNCTION(&_67$$18, "floatval", &_68, 153, &_72$$18);
		zephir_check_call_status();
		ZEPHIR_CALL_FUNCTION(&_73$$18, "floatval", &_68, 153, &bytes);
		zephir_check_call_status();
		if (ZEPHIR_GT(&_67$$18, &_73$$18)) {
			...
			ZEPHIR_CALL_FUNCTION(&_85$$20, "strtr", &_23, 76, &message, &replace);
			zephir_check_call_status();
	...
	if (zephir_is_true(&_86)) {
		...
		if ((zephir_function_exists_ex(SL("finfo_open") TSRMLS_CC) == SUCCESS)) {
			ZVAL_LONG(&_88$$26, 16);
			ZEPHIR_CALL_FUNCTION(&tmp, "finfo_open", NULL, 154, &_88$$26);
			zephir_check_call_status();
			zephir_array_fetch_string(&_89$$26, &value, SL("tmp_name"), PH_NOISY | PH_READONLY, "ice/validation/validator/file.zep", 171 TSRMLS_CC);
			ZEPHIR_CALL_FUNCTION(&mime, "finfo_file", NULL, 155, &tmp, &_89$$26);
			zephir_check_call_status();
			ZEPHIR_CALL_FUNCTION(NULL, "finfo_close", NULL, 156, &tmp);
			zephir_check_call_status();
		} else {
		...
		if (!(zephir_fast_in_array(&mime, &types TSRMLS_CC))) {
			...
			ZEPHIR_CALL_FUNCTION(&_101$$28, "strtr", &_23, 76, &message, &replace);
			zephir_check_call_status();
	...
	if (_103) {
		...
		ZEPHIR_CALL_FUNCTION(&tmp, "getimagesize", NULL, 157, &_105$$32);
		zephir_check_call_status();
		...
		if (_110$$32) {
			...
			ZEPHIR_CALL_FUNCTION(&_122$$35, "strtr", &_23, 76, &message, &replace);
			zephir_check_call_status();
		...
		if (zephir_is_true(&_123$$32)) {
			...
			if (_126$$39) {
				...
				ZEPHIR_CALL_FUNCTION(&_138$$40, "strtr", &_23, 76, &message, &replace);
				zephir_check_call_status();
