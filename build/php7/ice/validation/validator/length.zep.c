...
PHP_METHOD(Ice_Validation_Validator_Length, validate) {
	...
	if ((zephir_function_exists_ex(SL("mb_strlen") TSRMLS_CC) == SUCCESS)) {
		ZEPHIR_CALL_FUNCTION(&length, "mb_strlen", NULL, 176, &value);
		zephir_check_call_status();
	} else {
	...
	if (_11) {
		...
		ZEPHIR_CALL_FUNCTION(&_23$$12, "strtr", &_24, 88, &message, &replace);
		zephir_check_call_status();
	...
	if (_25) {
		...
		ZEPHIR_CALL_FUNCTION(&_37$$16, "strtr", &_24, 88, &message, &replace);
		zephir_check_call_status();
