...
PHP_METHOD(Ice_Validation_Validator_NotIn, validate)
{
	...
	if (Z_TYPE_P(&values) != IS_ARRAY) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(ice_exception_ce, "Values must be an array", "ice/validation/validator/notin.zep", 65);
		return;
	}
	if (zephir_fast_in_array(&value, &values)) {
		...
		ZEPHIR_CALL_FUNCTION(&_16$$6, "strtr", NULL, 111, &message, &replace);
		zephir_check_call_status();
