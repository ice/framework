...
PHP_METHOD(Ice_Auth_Social_Adapter, __construct) {
	...
	if (!(zephir_fast_count_int(config TSRMLS_CC))) {
		...
		ZEPHIR_CALL_METHOD(&_3, _1, "getconfig", NULL);
		zephir_check_call_status();
		ZEPHIR_INIT_VAR(_4);
		...
		zephir_check_call_status();
		_5 = zephir_is_true(tmp);
		if (_5) {
			_7 = zephir_fetch_nproperty_this(this_ptr, SL("_provider"), PH_NOISY_CC);
			ZEPHIR_CALL_METHOD(&_6, tmp, "has", NULL, _7);
			zephir_check_call_status();
			_5 = zephir_is_true(_6);
		}
		if (_5) {
			_9 = zephir_fetch_nproperty_this(this_ptr, SL("_provider"), PH_NOISY_CC);
			ZEPHIR_CALL_METHOD(&_8, tmp, "get", NULL, _9);
			zephir_check_call_status();
			ZEPHIR_CALL_METHOD(&config, _8, "toarray", NULL);
			zephir_check_call_status();
		}
	}
	...
	_7 = zephir_fetch_nproperty_this(this_ptr, SL("_clientId"), PH_NOISY_CC);
	_5 = !zephir_is_true(_7);
	if (!(_5)) {
		_9 = zephir_fetch_nproperty_this(this_ptr, SL("_clientSecret"), PH_NOISY_CC);
		_5 = !zephir_is_true(_9);
	}
	_10 = _5;
	if (!(_10)) {
		_11 = zephir_fetch_nproperty_this(this_ptr, SL("_redirectUri"), PH_NOISY_CC);
	...
	if (_10) {
		...
		ZVAL_STRING(_13, "client_id", 1);
		zephir_array_fast_append(_12, _13);
		ZEPHIR_INIT_NVAR(_13);
		ZVAL_STRING(_13, "client_secret", 1);
		zephir_array_fast_append(_12, _13);
		ZEPHIR_INIT_NVAR(_13);
		...
		zephir_check_call_status();
		zephir_throw_exception_debug(_4, "ice/auth/social/adapter.zep", 59 TSRMLS_CC);
		ZEPHIR_MM_RESTORE();
		return;
	}
	ZEPHIR_MM_RESTORE();
...
PHP_METHOD(Ice_Auth_Social_Adapter, has) {
	...
	if (zephir_array_isset(_0, key)) {
		...
		zephir_array_fetch(&_2, _1, key, PH_NOISY | PH_READONLY, "ice/auth/social/adapter.zep", 96 TSRMLS_CC);
...
PHP_METHOD(Ice_Auth_Social_Adapter, get) {
	...
	if (zephir_array_isset(_0, key)) {
		_1 = zephir_fetch_nproperty_this(this_ptr, SL("_socialFieldsMap"), PH_NOISY_CC);
		zephir_array_fetch(&_2, _1, key, PH_NOISY | PH_READONLY, "ice/auth/social/adapter.zep", 116 TSRMLS_CC);
		zephir_get_strval(key, _2);
	}
	_1 = zephir_fetch_nproperty_this(this_ptr, SL("_userInfo"), PH_NOISY_CC);
...
PHP_METHOD(Ice_Auth_Social_Adapter, getAuthUrl) {
	...
	zephir_check_call_status();
	zephir_array_fetch_string(&_0, config, SL("auth_url"), PH_NOISY | PH_READONLY, "ice/auth/social/adapter.zep", 137 TSRMLS_CC);
	zephir_array_fetch_string(&_1, config, SL("auth_params"), PH_NOISY | PH_READONLY, "ice/auth/social/adapter.zep", 137 TSRMLS_CC);
