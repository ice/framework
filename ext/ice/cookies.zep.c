...
PHP_METHOD(Ice_Cookies, salt) {
	...
	ZEPHIR_RETURN_CALL_FUNCTION("sha1", NULL, 42, _7);
	zephir_check_call_status();
	RETURN_MM();
...
PHP_METHOD(Ice_Cookies, setcookie) {
	...
	ZEPHIR_SINIT_VAR(_0);
	ZVAL_LONG(&_0, expire);
	ZEPHIR_RETURN_CALL_FUNCTION("setcookie", NULL, 62, name, value, &_0, path, domain, (secure ? ZEPHIR_GLOBAL(global_true) : ZEPHIR_GLOBAL(global_false)), (httpOnly ? ZEPHIR_GLOBAL(global_true) : ZEPHIR_GLOBAL(global_false)));
	zephir_check_call_status();
	RETURN_MM();
