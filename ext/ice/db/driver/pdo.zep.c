...
PHP_METHOD(Ice_Db_Driver_Pdo, __construct) {
	...
	if (_2) {
		...
		ZEPHIR_SINIT_VAR(_4);
		ZVAL_STRING(&_4, ":", 0);
		ZEPHIR_CALL_FUNCTION(&_5, "strstr", NULL, 65, _3, &_4, ZEPHIR_GLOBAL(global_true));
					...
					if (ZEPHIR_IS_STRING(operator, "OR") || ZEPHIR_IS_STRING(operator, "or") || ZEPHIR_IS_STRING(operator, "$or")) {
						...
						for (
						  ...
						  ; zephir_hash_move_forward_ex(_6, &_5)
							...
							Z_SET_ISREF_P(item);
							ZEPHIR_CALL_FUNCTION(&key, "key", &_8, 66, item);
							Z_UNSET_ISREF_P(item);
							zephir_check_call_status();
							Z_SET_ISREF_P(item);
							ZEPHIR_CALL_FUNCTION(&value, "current", &_9, 67, item);
							Z_UNSET_ISREF_P(item);
							zephir_check_call_status();
							if (Z_TYPE_P(value) == IS_ARRAY) {
								Z_SET_ISREF_P(value);
								ZEPHIR_CALL_FUNCTION(&is, "key", &_8, 66, value);
								Z_UNSET_ISREF_P(value);
								zephir_check_call_status();
								Z_SET_ISREF_P(value);
								ZEPHIR_CALL_FUNCTION(&_10, "current", &_9, 67, value);
								Z_UNSET_ISREF_P(value);
								zephir_check_call_status();
...
PHP_METHOD(Ice_Db_Driver_Pdo, select) {
	...
	ZEPHIR_CALL_METHOD(&filtered, this_ptr, "where", NULL, 68, filters);
	zephir_check_call_status();
...
PHP_METHOD(Ice_Db_Driver_Pdo, update) {
	...
	ZEPHIR_CALL_METHOD(&filtered, this_ptr, "where", NULL, 68, filters, values);
	zephir_check_call_status();
	ZEPHIR_INIT_VAR(_5);
	zephir_fast_join_str(_5, SL(", "), columns TSRMLS_CC);
