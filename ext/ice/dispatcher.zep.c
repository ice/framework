...
PHP_METHOD(Ice_Dispatcher, dispatch) {
	...
	while (1) {
		...
		if (zephir_is_true(modules)) {
			...
			ZEPHIR_CALL_METHOD(&_6, loader, "addnamespace", &_14, 27, moduleNamespace, path);
			zephir_check_call_status();
		...
		if (!(zephir_class_exists(handlerClass, 1 TSRMLS_CC))) {
			...
			zephir_check_call_status();
			zephir_throw_exception_debug(_10, "ice/dispatcher.zep", 180 TSRMLS_CC);
			ZEPHIR_MM_RESTORE();
			return;
		}
		...
		if (!((zephir_method_exists(handler, actionMethod TSRMLS_CC)  == SUCCESS))) {
			...
			zephir_check_call_status();
			zephir_throw_exception_debug(_9, "ice/dispatcher.zep", 212 TSRMLS_CC);
			ZEPHIR_MM_RESTORE();
			return;
		}
		...
		if (Z_TYPE_P(params) != IS_ARRAY) {
			...
			zephir_check_call_status();
			zephir_throw_exception_debug(_9, "ice/dispatcher.zep", 217 TSRMLS_CC);
			ZEPHIR_MM_RESTORE();
			return;
		}
		ZEPHIR_INIT_NVAR(_9);
