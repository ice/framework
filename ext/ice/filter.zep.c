...
PHP_METHOD(Ice_Filter, doSanitize) {
	...
	do {
		...
		if (ZEPHIR_IS_STRING(filter, "human")) {
			...
			ZEPHIR_CALL_FUNCTION(&_8, "preg_replace", &_9, 98, _6, _7, value);
		...
		if (ZEPHIR_IS_STRING(filter, "lower")) {
			...
			if ((zephir_function_exists_ex(SS("mb_strtolower") TSRMLS_CC) == SUCCESS)) {
				ZEPHIR_CALL_FUNCTION(&_3, "mb_strtolower", NULL, 99, value);
				zephir_check_call_status();
			} else {
		...
		if (ZEPHIR_IS_STRING(filter, "upper")) {
			...
			if ((zephir_function_exists_ex(SS("mb_strtoupper") TSRMLS_CC) == SUCCESS)) {
				ZEPHIR_CALL_FUNCTION(&_3, "mb_strtoupper", NULL, 100, value);
				zephir_check_call_status();
			} else {
		...
		if (ZEPHIR_IS_STRING(filter, "alnum")) {
			...
			ZEPHIR_RETURN_CALL_FUNCTION("preg_replace", &_9, 98, _3, _4, value);
		...
		if (ZEPHIR_IS_STRING(filter, "email")) {
			ZEPHIR_SINIT_NVAR(_2);
			ZVAL_LONG(&_2, 517);
			ZEPHIR_RETURN_CALL_FUNCTION("filter_var", &_11, 101, value, &_2);
			zephir_check_call_status();
			RETURN_MM();
		}
		if (ZEPHIR_IS_STRING(filter, "float")) {
			...
			ZVAL_LONG(&_10, 4096);
			ZEPHIR_CALL_FUNCTION(&_8, "filter_var", &_11, 101, value, &_2, &_10);
			zephir_check_call_status();
		...
		if (ZEPHIR_IS_STRING(filter, "int")) {
			ZEPHIR_SINIT_NVAR(_2);
			ZVAL_LONG(&_2, 519);
			ZEPHIR_CALL_FUNCTION(&_8, "filter_var", &_11, 101, value, &_2);
			zephir_check_call_status();
		...
		if (ZEPHIR_IS_STRING(filter, "repeats") || ZEPHIR_IS_STRING(filter, "strip_repeats")) {
			...
			ZEPHIR_RETURN_CALL_FUNCTION("preg_replace", &_9, 98, _1, _12, value);
			zephir_check_call_status();
			RETURN_MM();
		}
		if (ZEPHIR_IS_STRING(filter, "e") || ZEPHIR_IS_STRING(filter, "escape") || ZEPHIR_IS_STRING(filter, "strip_special")) {
			...
			ZEPHIR_SINIT_NVAR(_2);
			ZVAL_LONG(&_2, (3 | 48));
			ZEPHIR_RETURN_CALL_FUNCTION("htmlspecialchars", NULL, 102, _14, &_2);
			zephir_check_call_status();
			RETURN_MM();
		}
		if (ZEPHIR_IS_STRING(filter, "unescape") || ZEPHIR_IS_STRING(filter, "unstrip_special")) {
			ZEPHIR_SINIT_NVAR(_2);
			ZVAL_LONG(&_2, (3 | 48));
			ZEPHIR_RETURN_CALL_FUNCTION("htmlspecialchars_decode", NULL, 103, value, &_2);
			zephir_check_call_status();
			RETURN_MM();
		}
		ZEPHIR_INIT_NVAR(_3);
