...
PHP_METHOD(Ice_Loader, addNamespace) {
	...
	if (!(zephir_array_isset(_9, prefix))) {
		ZEPHIR_INIT_VAR(_10);
		array_init(_10);
		zephir_update_property_array(this_ptr, SL("prefixes"), prefix, _10 TSRMLS_CC);
	}
	if (prepend) {
		...
		ZEPHIR_CALL_FUNCTION(&_13, "utf8_encode", &_14, 112, baseDir);
		zephir_check_call_status();
		Z_SET_ISREF_P(_12);
		ZEPHIR_CALL_FUNCTION(NULL, "array_unshift", NULL, 113, _12, _13);
		Z_UNSET_ISREF_P(_12);
		zephir_check_call_status();
	} else {
		_11 = zephir_fetch_nproperty_this(this_ptr, SL("prefixes"), PH_NOISY_CC);
		...
		zephir_check_call_status();
		Z_SET_ISREF_P(_12);
		ZEPHIR_CALL_FUNCTION(NULL, "array_push", NULL, 114, _12, _13);
		Z_UNSET_ISREF_P(_12);
		zephir_check_call_status();
	}
	RETURN_THIS();
...
PHP_METHOD(Ice_Loader, loadClass) {
	...
	while (1) {
		...
		ZVAL_STRING(&_7, "\\", 0);
		ZEPHIR_CALL_FUNCTION(&pos, "strrpos", &_1, 115, prefix, &_7);
		zephir_check_call_status();
	}
	RETURN_MM_BOOL(0);
