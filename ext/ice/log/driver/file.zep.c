...
PHP_METHOD(Ice_Log_Driver_File, __construct) {
	...
	if (!((zephir_file_exists(file TSRMLS_CC) == SUCCESS))) {
		ZEPHIR_CALL_FUNCTION(&_0, "touch", &_1, file);
...
PHP_METHOD(Ice_Log_Driver_File, log) {
	...
	ZEPHIR_CALL_METHOD(&_3, this_ptr, "interpolate", NULL, message, context);
	zephir_check_call_status();
	ZEPHIR_INIT_VAR(_4);
	...
	ZVAL_LONG(&_0, 8);
	ZEPHIR_CALL_FUNCTION(NULL, "file_put_contents", &_6, _5, line, &_0);
	zephir_check_call_status();
	ZEPHIR_MM_RESTORE();
