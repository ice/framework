...
PHP_METHOD(Ice_Log_Driver_File, __construct) {
	...
	if (!((zephir_file_exists(file TSRMLS_CC) == SUCCESS))) {
		ZEPHIR_CALL_FUNCTION(&_0, "touch", NULL, 101, file);
	...
	ZEPHIR_CALL_FUNCTION(&_0, "is_writable", NULL, 102, file);
	zephir_check_call_status();
	if (!(zephir_is_true(_0))) {
		ZEPHIR_INIT_NVAR(_1);
		object_init_ex(_1, ice_exception_ce);
...
PHP_METHOD(Ice_Log_Driver_File, log) {
	...
	ZEPHIR_CALL_FUNCTION(&_1, "date", NULL, 32, &_0);
	zephir_check_call_status();
	ZEPHIR_INIT_VAR(_2);
	...
	ZVAL_LONG(&_0, 8);
	ZEPHIR_CALL_FUNCTION(NULL, "file_put_contents", NULL, 103, _5, line, &_0);
	zephir_check_call_status();
	ZEPHIR_MM_RESTORE();
...
PHP_METHOD(Ice_Log_Driver_File, interpolate) {
	...
	ZEPHIR_RETURN_CALL_FUNCTION("strtr", NULL, 67, message, replace);
	zephir_check_call_status();
	RETURN_MM();
