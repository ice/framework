...
PHP_METHOD(Ice_Validation_Validator_File, validate) {
	...
	if (_7) {
		...
		ZEPHIR_CALL_FUNCTION(&_15, "strtr", &_17, 82, message, replace);
		zephir_check_call_status();
	...
	if (!(_23)) {
		...
		ZEPHIR_CALL_FUNCTION(&_10, "is_uploaded_file", NULL, 162, _24);
		zephir_check_call_status();
	...
	if (zephir_is_true(_10)) {
		...
		ZEPHIR_CALL_FUNCTION(&_11, "floatval", &_32, 163, _31);
		zephir_check_call_status();
		ZEPHIR_INIT_VAR(_33);
		...
		ZEPHIR_CALL_FUNCTION(&_11, "floatval", &_32, 163, _36);
		zephir_check_call_status();
		ZEPHIR_CALL_FUNCTION(&_13, "floatval", &_32, 163, bytes);
		zephir_check_call_status();
		if (ZEPHIR_GT(_11, _13)) {
			...
			ZEPHIR_CALL_FUNCTION(&_37, "strtr", &_17, 82, message, replace);
			zephir_check_call_status();
	...
	if (zephir_is_true(_10)) {
		...
		if ((zephir_function_exists_ex(SS("finfo_open") TSRMLS_CC) == SUCCESS)) {
			...
			ZVAL_LONG(&_35, 16);
			ZEPHIR_CALL_FUNCTION(&tmp, "finfo_open", NULL, 164, &_35);
			zephir_check_call_status();
			zephir_array_fetch_string(&_31, value, SL("tmp_name"), PH_NOISY | PH_READONLY, "ice/validation/validator/file.zep", 171 TSRMLS_CC);
			ZEPHIR_CALL_FUNCTION(&mime, "finfo_file", NULL, 165, tmp, _31);
			zephir_check_call_status();
			ZEPHIR_CALL_FUNCTION(NULL, "finfo_close", NULL, 166, tmp);
			zephir_check_call_status();
		} else {
		...
		if (!(zephir_fast_in_array(mime, types TSRMLS_CC))) {
			...
			ZEPHIR_CALL_FUNCTION(&_26, "strtr", &_17, 82, message, replace);
			zephir_check_call_status();
	...
	if (_12) {
		...
		ZEPHIR_CALL_FUNCTION(&tmp, "getimagesize", NULL, 167, _31);
		zephir_check_call_status();
		...
		if (zephir_is_true(_15)) {
			...
			if (_40) {
				...
				ZEPHIR_CALL_FUNCTION(&_43, "strtr", &_17, 82, message, replace);
				zephir_check_call_status();
